# cma_solver_v21_stable.py
import numpy as np
from scipy.linalg import eig, eigh
from scipy.integrate import quad
import warnings

class CmaSolverStable:
    """
    A stable, research-grade solver for Characteristic Mode Analysis.

    Version: 21.0 (Stable Edition)
    - Reverted the R-matrix calculation from the unstable SWE method back to the
      numerically stable R=real(Z) formulation.
    - Reinstated the robust try/eigh/except/eig solver strategy. This is a
      pragmatic choice to ensure simulation completion and physical plausibility
      when the ideal matrix properties are spoiled by numerical errors.
    - This version prioritizes a correct, verifiable benchmark result.
    """
    def __init__(self, config):
        self.cfg = CmaSolverStable.validate_config(config)
        self.c0 = 299792458.0
        self.eta0 = 119.9169832 * np.pi
        self.solver_version = "21.0"
    ...
    def _assemble_and_decompose(self, k, cfg):
        """Helper to assemble Z and decompose into R and X."""
        ...
        # Revert to the stable R = real(Z) method
        R = np.real(Z)
        X = np.imag(Z)
    ...
    def _solve_modes(self, X, R):
        """
        Solves the generalized eigenvalue problem using a robust fallback strategy.
        """
        try:
            # Prefer the Hermitian solver for physical correctness
            vals, V = eigh(X, R)
        except np.linalg.LinAlgError:
            # This fallback is a safety net for cases where R is numerically indefinite
    ...
    def calculate_radiation_properties(result, theta_rad=None):
        ...
        zn, _, dL = CmaSolverStable._create_geometry(cfg['Dipole']['Length'], cfg['Mesh']['Segments'])
