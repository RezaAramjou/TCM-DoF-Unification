...
class CmaSolver:
    ...
    Version: 11.1 (Rock-Solid, Final)
    - Corrected final AttributeError for _get_tolerances by reading from config.
    ...
    def _run_single_frequency(self, f):
        ...
        Z_in_n = self._calculate_input_impedance(Z, J_n, zc, zn, dL, lambda_n)
    ...
    def _calculate_input_impedance(self, Z, Jn, zc, zn, dL, lambda_n):
        ...
        alpha = Jn.conj().T @ V
        Z_in = np.sum(alpha**2 / (1 + 1j*lambda_n))
        return np.array([Z_in] * len(lambda_n)) # Return array for consistency
