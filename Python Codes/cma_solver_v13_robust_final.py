# cma_solver_v13_robust_final.py
import numpy as np
...
class CmaSolver:
    ...
    Version: 13.3 (Robust Final)
    - Implemented more aggressive stabilization for the resistance matrix (R)
      to finally resolve the "not positive definite" error.
    - Added explicit clipping of negative diagonal elements in R.
    - Regularization is now scaled relative to the max element of R.
    - Added safe normalization for eigenvectors to prevent sqrt of negative numbers.
    """
    def __init__(self, config):
        self.cfg = CmaSolver.validate_config(config)
        self.c0 = 299792458.0
        self.eta0 = 119.9169832 * np.pi
        self.solver_version = "13.3"
    ...
    def _solve_modes(self, X, R):
        ...
        # --- STABILITY FIX 1: Force diagonal of R to be non-negative ---
        # This corrects for small negative values from numerical precision errors.
        np.fill_diagonal(R, np.maximum(np.diag(R), 0))

        # --- STABILITY FIX 2: More robust diagonal loading ---
        # This regularization prevents the Cholesky decomposition from failing on
        # matrices that are numerically close to singular.
        ...
        try:
            ...
            raise np.linalg.LinAlgError(
                "Resistance matrix R is not positive definite, even after robust regularization. "
                "CMA is not physically meaningful. There may be a fundamental error in the Z-matrix calculation."
            )
