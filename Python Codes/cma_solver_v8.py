...
class CmaSolver:
    ...
    Version: 8.0 (Rock-Solid, High-Performance)
    - API refactored into a clean, hierarchical config structure.
    - Replaced slow numerical integration with a high-performance Gauss-Legendre
      quadrature scheme, including an analytical self-term for accuracy.
    - Implemented robust matrix condition checking and a stable eigensolver fallback.
    - Added memory-saving options for large sweeps.
    - Corrected all identified scientific and implementation flaws.
    """
    def __init__(self, config):
        self.cfg = CmaSolver.validate_config(config)
        self.c0 = 299792458.0
        self.eta0 = 119.9169832 * np.pi
        self.solver_version = "8.0"
    ...
    def _calculate_Z_element(self, m, n, zn, zc, dL, k, a):
        ...
        if basis == "rooftop":
            ...
            if m == n: # Analytical self-term for vector potential
                L_seg = dL[m] + dL[m+1]
                term_A = (1j*self.eta0*k/(6*np.pi)) * (np.log(L_seg/a) - 1.75) * L_seg
            else:
    ...
    def _solve_modes(self, X, R):
        ...
        if np.linalg.cond(R) > 1e15:
            warnings.warn(f"Resistance matrix is ill-conditioned (cond={np.linalg.cond(R):.2e}). "
                          "Falling back to general eigensolver. Higher-order modes may be inaccurate.", UserWarning)
            vals, V = eig(X, R)
        else:
            try:
        ...
        # R-orthonormalize eigenvectors: J_n^T * R * J_m = delta_mn
    ...
    def calculate_radiation_properties(result, theta_rad=None):
        ...
        J_n, k, zn = result['J_n'], result['wavenumber'], result['z_nodes']
