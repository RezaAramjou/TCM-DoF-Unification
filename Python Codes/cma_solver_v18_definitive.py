...
class CmaSolverDefinitive:
    ...
    Version: 18.0 (Definitive Edition)
    - Implemented high-accuracy singularity handling via full sub-patch quadrature
      for both vector and scalar potentials in the self-impedance term.
    - Clarified and confirmed the use of the rigorous, energy-based modal Q-factor
      calculation via frequency-swept dZ/d(omega).
    - Corrected and clarified the basis function count for rooftops (N = Segments - 1).
    - Uses a robust try/eigh/except/eig solver strategy with clear documentation
      on the physics vs. numerical stability trade-off.
    """
    def __init__(self, config):
        self.cfg = CmaSolverDefinitive.validate_config(config)
        self.c0 = 299792458.0
        self.eta0 = 119.9169832 * np.pi
        self.solver_version = "18.0"
    ...
    def run(self):
        ...
        for i in range(J_n.shape[1]):
            ...
            # This is the rigorous, energy-based Q-factor definition:
            # Q_n = |omega * (Wm - We)| / P_rad
            # where 2 * omega * (Wm - We) is approximated by I.H * (omega*dX/d(omega) - X)I
            # A more direct and stable formula is Q_n = |w/2 * I.H*dZ/dw*I| / (I.H*R*I)
    ...
    def _assemble_impedance(self, k, cfg):
        ...
        # For a linear dipole with S segments, there are S+1 nodes. Rooftop basis
        # functions are centered on the S-1 internal nodes.
    ...
    def _calculate_self_term(self, m, zn, dL, k, a):
        """
        Calculates the self-impedance Z_mm using full sub-patch quadrature for
        both vector and scalar potential terms to accurately handle the singularity.
        """
        num_sub_patches = 10 # Increase for more accuracy
        integral_A = 0
        integral_V = 0
        
        q_nodes, q_weights = np.polynomial.legendre.leggauss(self.nq)

        # Sub-patching for the vector potential term (A)
        z_m_support_start, z_m_support_end = zn[m], zn[m+2]
        sub_patch_nodes_A = np.linspace(z_m_support_start, z_m_support_end, num_sub_patches + 1)
        for i in range(num_sub_patches):
            for j in range(num_sub_patches):
                integral_A += self._gauss_quad_2d(
                    lambda z, zp: self._rooftop(z, zn[m+1], dL[m], dL[m+1]) * \
                                  self._green(z, zp, k, a) * \
                                  self._rooftop(zp, zn[m+1], dL[m], dL[m+1]),
                    (sub_patch_nodes_A[i], sub_patch_nodes_A[i+1]),
                    (sub_patch_nodes_A[j], sub_patch_nodes_A[j+1]),
                    q_nodes, q_weights
                )
        
        # Sub-patching for the scalar potential term (V)
        # This involves four separate 2D integrals
        term_V_1, term_V_2, term_V_3, term_V_4 = 0, 0, 0, 0
        
        # Integral 1: m, m
        nodes1 = np.linspace(zn[m], zn[m+1], num_sub_patches + 1)
        for i in range(num_sub_patches):
            for j in range(num_sub_patches):
                term_V_1 += self._gauss_quad_2d(lambda z, zp: self._green(z, zp, k, a), (nodes1[i], nodes1[i+1]), (nodes1[j], nodes1[j+1]), q_nodes, q_weights)

        # Integral 2: m, m+1
        nodes2 = np.linspace(zn[m+1], zn[m+2], num_sub_patches + 1)
        for i in range(num_sub_patches):
            for j in range(num_sub_patches):
                term_V_2 += self._gauss_quad_2d(lambda z, zp: self._green(z, zp, k, a), (nodes1[i], nodes1[i+1]), (nodes2[j], nodes2[j+1]), q_nodes, q_weights)

        # Integral 3: m+1, m
        for i in range(num_sub_patches):
            for j in range(num_sub_patches):
                term_V_3 += self._gauss_quad_2d(lambda z, zp: self._green(z, zp, k, a), (nodes2[i], nodes2[i+1]), (nodes1[j], nodes1[j+1]), q_nodes, q_weights)

        # Integral 4: m+1, m+1
        for i in range(num_sub_patches):
            for j in range(num_sub_patches):
                term_V_4 += self._gauss_quad_2d(lambda z, zp: self._green(z, zp, k, a), (nodes2[i], nodes2[i+1]), (nodes2[j], nodes2[j+1]), q_nodes, q_weights)

        const_A = 1j * k * self.eta0 / (4 * np.pi)
        const_V = self.eta0 / (1j * k * 4 * np.pi)
        
        final_term_A = const_A * integral_A
        final_term_V = const_V * ((term_V_1/(dL[m]*dL[m])) - (term_V_2/(dL[m]*dL[m+1])) - (term_V_3/(dL[m+1]*dL[m])) + (term_V_4/(dL[m+1]*dL[m+1])))
    ...
    def _solve_modes(self, X, R):
        ...
        This approach prefers the physically correct Hermitian solver but falls
        back to a general solver if numerical errors spoil the matrix properties,
        ensuring the simulation completes.
        """
        try:
            vals, V = eigh(X, R)
