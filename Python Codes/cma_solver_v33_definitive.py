...
class CmaSolverV33:
    ...
    Version: 33.0 (Definitive & Tracked)
    - Theoretical Context: Docstrings updated to reflect the choice of the
      Harrington & Mautz impedance operator formulation (EFIE) suitable for
      open structures like dipoles, and to acknowledge its relationship to the
      more general Garbacz scattering formulation.
    - Smooth Bessel Blending: Implements a smooth tanh blending function to
      transition between scipy's Bessel functions and a stable asymptotic form.
    - Configurable Regularization & Tracking: The regularization factor for the GEP
      eigensolver and the correlation threshold for mode tracking are now
      user-configurable parameters.
    """
    def __init__(self, config):
        self.cfg = CmaSolverV33.validate_config(config)
        self.c0 = 299792458.0
        self.eta0 = 119.9169832 * np.pi
        self.solver_version = "33.0"
    ...
    def _calculate_input_impedance(self, Z):
        ...
        for i in range(N_basis):
            dist_from_center = np.abs(z_centers[i])
    ...
    def _stable_bessel_integrand(n, k, z):
        ...
        if weight > 0.999: # Fully asymptotic
    ...
    def validate_config(cfg):
        d = {
            ...
            'Mesh': {'Segments': 51},
            'Feed': {'GapWidth': 0.005},
            'Execution': {'Frequency': 300e6, 'NumModes': 10, 'Verbose': False, 'PowerFilterThreshold': 1e-7, 'ModeTrackingThreshold': 0.9},
