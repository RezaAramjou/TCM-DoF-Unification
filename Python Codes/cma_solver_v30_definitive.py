...
class CmaSolverV30:
    ...
    Version: 30.0 (Definitive & Stabilized)
    - Eigensolver Regularization: Implements a try/except block that applies
      a small diagonal loading (regularization) to the R matrix if the initial
      eigensolver call fails, ensuring numerical stability.
    - Robust Integration: Wraps the core `quad` call in the SWE reconstruction
      to catch potential integration errors gracefully without halting.
    - All features from v29 are retained.
    """
    def __init__(self, config):
        self.cfg = CmaSolverV30.validate_config(config)
        self.c0 = 299792458.0
        self.eta0 = 119.9169832 * np.pi
        self.solver_version = "30.0"
    ...
    def _solve_and_filter_modes(self, X, R):
        """Solves GEP, applying regularization if the initial attempt fails."""
        try:
            # First attempt
            vals, V = eigh(X, R)
        except np.linalg.LinAlgError:
            # If it fails, apply regularization and try again
            warnings.warn("eigh failed due to non-positive definite R. Applying regularization.", UserWarning)
            # Add a small identity matrix scaled by the max value of R to the diagonal
            R_reg = R + np.eye(R.shape[0]) * 1e-12 * np.max(np.abs(R))
            try:
                vals, V = eigh(X, R_reg)
