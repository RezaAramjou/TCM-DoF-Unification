...
for r in tqdm(results, desc="Analyzing Results"):
    ...
    _, E_pattern_matrix, _ = CmaSolver.calculate_radiation_properties(
        {'VersionInfo': r['VersionInfo'], 'J_n': I_total[:, np.newaxis], 'wavenumber': r['wavenumber'], 'z_nodes': r['z_nodes']}
    )
