...
from cma_solver_v17_research import CmaSolverResearch 
...
def p_lm_stable(l, m, x):
    ...
    three-term recurrence relations, avoiding direct computation that can
    be unstable for large l.
    """
...
def fit_spherical_waves(theta, E_pattern_complex, max_N, error_threshold):
    ...
    """
    n_gauss_points = 512 # High-density grid for accurate fitting
...
def run_analysis_for_length(L, f0, a_const, N_const, feed_gap):
    ...
    solver = CmaSolverResearch(config)
    return solver.run()

# --- Step 1: Define Sweep Parameters ---
print('--- Unification & Q-Factor Analysis (v17.0 Research Edition) ---')
f0 = 300e6
...
for r in tqdm(results, desc="Analyzing Results"):
    ...
    zn, _, _ = CmaSolverResearch._create_geometry(r['config']['Dipole']['Length'], r['config']['Mesh']['Segments'])
    center_node_idx = np.argmin(np.abs(zn))
    V_impressed[center_node_idx] = 1.0
    alpha = r['J_n'].conj().T @ V_impressed
    I_total = r['J_n'] @ (alpha / (1 + 1j*r['lambda_n']))
    
    theta = np.linspace(1e-6, np.pi - 1e-6, 361)
    E_theta_complex = CmaSolverResearch.calculate_radiation_properties(
...
styles = CmaSolverResearch.get_plot_styles()
fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 12))
fig.suptitle('CMA Unification & Q-Factor Analysis (v17.0 Research Edition)', fontsize=16, fontweight='bold')
...
plt.savefig('Fig_Report_v17_Research.png', dpi=300)

print('Research report saved to Fig_Report_v17_Research.png. Analysis is complete.')
plt.show()
