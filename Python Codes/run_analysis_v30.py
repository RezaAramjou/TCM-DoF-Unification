...
from cma_solver_v30_definitive import CmaSolverV30, CMAError
import warnings
import json
import os
...
def run_mesh_convergence_automation(f0, a_const):
    ...
    print("\n--- Running Automated Mesh Convergence (this may take a long time)... ---")
        ...
        if len(impedances) > 1 and not np.isnan(impedances[-1]) and not np.isnan(impedances[-2]):
            ...
            if z_change < 1.0: # 1 Ohm convergence tolerance
                ...
                with open(CACHE_FILE, 'w') as f:
                    json.dump({'segments_per_lambda': segs_per_lambda, 'f0_MHz': f0/1e6, 'a_const_m': a_const}, f)
    ...
    final_segs_per_lambda = int(segment_counts[-1] / (L_test/lambda0))
    with open(CACHE_FILE, 'w') as f:
        json.dump({'segments_per_lambda': final_segs_per_lambda, 'f0_MHz': f0/1e6, 'a_const_m': a_const}, f)
    print(f"  Result saved to {CACHE_FILE}.")
    return final_segs_per_lambda

def run_validation_suite(f0, a_const, segs_per_lambda):
    """Runs a series of tests to validate the solver's integrity."""
    print("\n--- Running Validation Suite for v30 Solver ---")
    lambda0 = 299792458.0 / f0
    ...
    if res and len(res['P_rad_R']) > 0:
        ...
        rel_err = np.abs(P_R - P_FF) / np.abs(P_R) if P_R > 0 else 0
...
# This is the standard way to protect the main script execution
# in a parallel processing context, especially on Windows.
if __name__ == '__main__':
    # --- Step 1: Define Global Parameters ---
    print('--- CMA Definitive Analysis (v30.0) ---')
    f0 = 300e6
    lambda0 = 299792458.0 / f0
    ...
    if os.path.exists(CACHE_FILE):
        with open(CACHE_FILE, 'r') as f:
            ...
            segments_per_lambda = cache_data['segments_per_lambda']
            print(f"\n--- Loaded converged mesh density from {CACHE_FILE}: {segments_per_lambda} segs/λ ---")
    else:
    ...
    fig.suptitle('CMA Modal Q-Factor (v30.0 Definitive & Validated Solver)', fontsize=16, fontweight='bold')
    ...
    plt.savefig('Fig_Report_v30_Definitive.png', dpi=300)
    print('\nDefinitive report saved to Fig_Report_v30_Definitive.png.')
    plt.show()
