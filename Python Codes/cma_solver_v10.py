...
class CmaSolver:
    ...
    in Python, refactored to address all points from a detailed scientific review.

    Version: 10.0 (Rock-Solid, Final)
    - Completed the pulse-basis implementation with a correct MPIE formulation.
    - Replaced approximate self-term with accurate singularity subtraction.
    - Implemented robust eigensolver logic (eigh) and correct R-orthonormalization.
    - Implemented a physically consistent feed model for rooftop basis.
    ...
    def _run_single_frequency(self, f):
        ...
        Z_in_n = self._calculate_input_impedance(Z, J_n, zc, zn, dL)
    ...
    def _calculate_Z_element(self, m, n, zn, zc, dL, k, a):
        ...
        if basis == "pulse":
            ...
            kernel = lambda zp: self._green(zm, zp, k, a)
            d_kernel = lambda zp: self._green_d(zm, zp, k, a)

            term_V_real, _ = quad(lambda zp: np.real(kernel(zp)), zn_start, zn_end, epsabs=abs_tol, epsrel=rel_tol)
            term_V_imag, _ = quad(lambda zp: np.imag(kernel(zp)), zn_start, zn_end, epsabs=abs_tol, epsrel=rel_tol)
            term_V = term_V_real + 1j * term_V_imag
            
            term_A = d_kernel(zn_end) - d_kernel(zn_start)
            
            return (1j * self.eta0 / (4 * np.pi * k)) * (k**2 * term_V + term_A)
    ...
    def _gauss_quad_2d(self, func, z_range, zp_range):
        ...
        vals = func(z_nodes[:, np.newaxis], zp_nodes[np.newaxis, :])
        real_part = np.sum(z_weights[:, np.newaxis] * zp_weights[np.newaxis, :] * np.real(vals))
        imag_part = np.sum(z_weights[:, np.newaxis] * zp_weights[np.newaxis, :] * np.imag(vals))
    ...
    def _solve_modes(self, X, R):
        ...
        try:
            ...
            warnings.warn(f"Resistance matrix is not positive definite or Hermitian (cond={np.linalg.cond(R):.2e}). "
                          "Falling back to general eigensolver.", UserWarning)
            vals, V = eig(X, R)
        ...
        # R-orthonormalize eigenvectors: J_n^H * R * J_m = delta_mn
    ...
    def _calculate_input_impedance(self, Z, Jn, zc, zn, dL):
        N = Jn.shape[0]
        ...
        if basis == "pulse":
            ...
            V[feed_idx] = 1.0 # True delta-gap voltage source
        else: # rooftop
            node_idx = np.argmin(np.abs(zn))
            if node_idx > 0: V[node_idx - 1] = 0.5
            if node_idx < N: V[node_idx]     = 0.5
        
        alpha = Jn.conj().T @ V
        Z_modal = np.diag(Jn.conj().T @ Z @ Jn)
        return Z_modal / (alpha * alpha.conj())
    ...
    def validate_config(user_cfg):
        d = {
            ...
            'Execution': {'Frequency': 300e6, 'NumModes': 4, 'Verbose': True, 'PlotVisible': False, 'StoreZMatrix': True},
        }
