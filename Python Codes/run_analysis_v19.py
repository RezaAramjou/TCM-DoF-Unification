...
from cma_solver_v19_pinnacle import CmaSolverPinnacle 
...
def run_analysis_for_length(L, f0, a_const, segments_per_lambda, feed_gap):
    ...
    # Dynamically calculate number of segments to maintain mesh density
    num_segments = int(L / lambda0 * segments_per_lambda)
    if num_segments % 2 == 0: num_segments += 1 # Ensure odd number of segments
    if num_segments < 11: num_segments = 11 # Minimum number of segments
    ...
    solver = CmaSolverPinnacle(config)
    return solver.run()

# --- Step 1: Define Sweep Parameters ---
print('--- Unification & Q-Factor Analysis (v19.0 Pinnacle Edition) ---')
f0 = 300e6
lambda0 = 299792458.0 / f0
a_const = 0.001 * lambda0
segments_per_lambda = 40 # Maintain a constant mesh density
feed_gap = 0.01 * lambda0
L_over_lambda_sweep = np.linspace(0.1, 1.5, 31)
Ls = L_over_lambda_sweep * lambda0

# --- Step 2: Run Parallel Sweep ---
print('Starting parallel analysis with controlled meshing...')
...
for r in tqdm(results, desc="Analyzing Results"):
    ...
    # --- Intrinsic DoF from Far-Field of the First Characteristic Mode ---
    theta = np.linspace(1e-6, np.pi - 1e-6, 361)
    # Get the current of the first (most significant) characteristic mode
    J_mode1 = r['J_n'][:, np.argsort(np.abs(r['lambda_n']))[0]][:, np.newaxis]
    
    E_theta_mode1 = CmaSolverPinnacle.calculate_radiation_properties(
...
styles = CmaSolverPinnacle.get_plot_styles()
fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 12))
fig.suptitle('CMA Unification & Q-Factor Analysis (v19.0 Pinnacle Edition)', fontsize=16, fontweight='bold')
...
plt.savefig('Fig_Report_v19_Pinnacle.png', dpi=300)

print('Pinnacle report saved to Fig_Report_v19_Pinnacle.png. Analysis is complete.')
plt.show()
