# cma_solver_v19_pinnacle.py
import numpy as np
from scipy.linalg import eig, eigh
from scipy.integrate import quad
import warnings

class CmaSolverPinnacle:
    """
    A pinnacle, research-grade solver for Characteristic Mode Analysis.

    Version: 19.0 (Pinnacle Edition)
    - Corrected Q-factor to use the rigorous, reactance-based formulation (dX/d(omega)).
    - Corrected impedance matrix assembly to enforce complex symmetry (Z=Z^T),
      which is fundamental for reciprocal media in the Method of Moments.
    - Implemented high-accuracy singularity handling via full sub-patch quadrature.
    - Uses a robust try/eigh/except/eig solver strategy.
    """
    def __init__(self, config):
        self.cfg = CmaSolverPinnacle.validate_config(config)
        self.c0 = 299792458.0
        self.eta0 = 119.9169832 * np.pi
        self.solver_version = "19.0"
    ...
    def run(self):
        ...
        _, _, X_minus = self._assemble_and_decompose(2*np.pi*frequencies[0]/self.c0, self.cfg)
        center_results = self._run_single_frequency(frequencies[1])
        _, _, X_plus = self._assemble_and_decompose(2*np.pi*frequencies[2]/self.c0, self.cfg)

        # Numerically compute dX/d(omega) using a central difference for Q-factor
        ...
        for i in range(J_n.shape[1]):
            I_n = J_n[:, i]
            P_rad_n = 0.5 * I_n.conj().T @ R_center @ I_n
            
            # Rigorous Q-factor: Q_n = 2 * omega * |Wm - We| / P_rad
            # where Wm - We = 1/4 * I.H * (dX/d(omega)) * I
            stored_energy_derivative_term = 0.25 * I_n.conj().T @ dX_domega @ I_n
    ...
    def _assemble_and_decompose(self, k, cfg):
        """Helper to assemble and decompose Z for the frequency sweep."""
        ...
        # For reciprocal media, MoM matrix is complex symmetric: Z = Z^T
        Z = Z + np.triu(Z, 1).T
    ...
    def calculate_radiation_properties(result, theta_rad=None):
        ...
        zn, _, dL = CmaSolverPinnacle._create_geometry(cfg['Dipole']['Length'], cfg['Mesh']['Segments'])
    ...
    @staticmethod
    def _decompose_Z(Z): 
        # For a symmetric Z, R and X are also symmetric.
