...
class CmaSolver:
    ...
    in Python. This version incorporates a stable Mixed-Potential Integral
    Equation (MPIE) formulation and other best practices.

    Version: 7.3 (High-Performance)
    - Replaced slow scipy.dblquad with a highly optimized, vectorized
      Gauss-Legendre quadrature for all integrations, dramatically
      improving performance.
    - Corrected complex integration issue and static method calls.
    """
    def __init__(self, config, quad_order=8):
        self.cfg = CmaSolver.validate_config(config)
        self.c0 = 299792458.0
        self.eta0 = 119.9169832 * np.pi
        self.solver_version = "7.3"
    ...
    def _run_single_frequency(self, f):
        ...
        Z_in_n = self._calculate_input_impedance(Z, J_n, zc, zn)
        ...
        result = {
            ...
            'Q_n': np.abs(lambda_n), 'MS_n': 1 / np.abs(1 + 1j * lambda_n),
            'InputImpedance_n': Z_in_n,
        ...
        if cfg['Execution']['Verbose']:
            P_rad_n, _, D_n = self.calculate_radiation_properties(result)
    ...
    def _assemble_impedance(self, k, zn, zc, dL, a):
        ...
        cfg = self.cfg['Numerics']
        N = len(zc) if cfg['BasisFunction'] == "pulse" else len(zn) - 2
    ...
    def _calculate_Z_element(self, m, n, zn, zc, dL, k, a):
        """Calculates a single Z_mn element using optimized quadrature."""
        basis = self.cfg['Numerics']['BasisFunction']
        
        if basis == "pulse":
            zm = zc[m]
            zn_start, zn_end = zn[n], zn[n+1]
            zp_nodes = (zn_end - zn_start) / 2 * self.q_nodes + (zn_end + zn_start) / 2
            zp_weights = (zn_end - zn_start) / 2 * self.q_weights
            
            integrand_vals = (k**2 * self._green(zm, zp_nodes, k, a) - self._green_d2(zm, zp_nodes, k, a))
            val = np.sum(zp_weights * integrand_vals)
            return (1j * self.eta0 / (4 * np.pi * k)) * val
        
        elif basis == "rooftop":
            # MPIE Formulation with 2D Gauss-Legendre Quadrature
    ...
    def _gauss_quad_2d(self, func, z_range, zp_range):
        ...
        z_nodes = (z_max - z_min) / 2 * self.q_nodes + (z_max + z_min) / 2
        z_weights = (z_max - z_min) / 2 * self.q_weights
        zp_nodes = (zp_max - zp_min) / 2 * self.q_nodes + (zp_max + zp_min) / 2
        zp_weights = (zp_max - zp_min) / 2 * self.q_weights
        
        integrand_vals = func(z_nodes[:, np.newaxis], zp_nodes[np.newaxis, :])
        return np.sum(z_weights[:, np.newaxis] * zp_weights[np.newaxis, :] * integrand_vals)
        
    def _solve_modes(self, X, R):
        """Robustly solves the generalized eigenvalue problem."""
        try:
            cholesky(R, lower=True)
            vals, V = eig(X, R)
        except np.linalg.LinAlgError:
            warnings.warn("Resistance matrix is not positive definite. Falling back to general eigensolver.", UserWarning)
            vals, V = eig(X, R)
        
        idx = np.argsort(np.abs(vals))
        vals, V = vals[idx], V[:, idx]
        
        max_vals = np.max(np.abs(V), axis=0)
        V = V / max_vals
        return vals, V

    def _calculate_input_impedance(self, Z, Jn, zc, zn):
        N = Jn.shape[0]
        V = np.zeros(N)
        if self.cfg['Numerics']['BasisFunction'] == "pulse":
        ...
        V[feed_idx] = 1.0
        alpha = Jn.conj().T @ V
        return alpha**2 / np.diag(Jn.conj().T @ Z @ Jn)
    ...
    def calculate_radiation_properties(result, theta_rad=None):
        ...
        if theta_rad is None:
            theta_rad = np.linspace(0, np.pi, 181)
        ...
        U = np.abs(AF * np.sin(theta_rad))**2 / (2 * eta0)
    ...
    @staticmethod
    def _green_d2(z, zp, k, a):
        R = np.sqrt((z - zp)**2 + a**2)
        g = np.exp(-1j * k * R) / R
        term1 = (1j*k*R - 1) / R**2
        term2 = 1 - 3 * ((z - zp) / R)**2
        return -g * (term1 * term2 - term1)

    @staticmethod
    def _rooftop(z, center, dL_m, dL_p):
        val = np.zeros_like(np.atleast_1d(z))
    ...
    def _validate_physical(self, lambda_):
        ...
        if a/lambda_ > 1/500:
            raise ValueError(f"Thin-wire violation: a/λ must be < 1/500. Current value is {a/lambda_:.4f}.")
        if L/a < 50:
            raise ValueError(f"Thin-wire violation: L/a must be > 50. Current value is {L/a:.2f}.")
    ...
    def _log_summary_table(self, r):
        if not self.cfg['Execution']['Verbose']: return
        print("Modal Analysis Summary:")
        header = f"{'Mode':>5} {'Eigenvalue':>12} {'MS':>12} {'Q_Factor':>12} {'Directivity':>12} {'Z_in':>25}"
        print(header + "\n" + "-"*len(header))
        for i in range(min(10, len(r['lambda_n']))):
            print(f"{i+1:5d} {r['lambda_n'][i]:12.4g} {r['MS_n'][i]:12.4g} {r['Q_n'][i]:12.4g} "
                  f"{r['Directivity_n'][i]:12.4f} {r['InputImpedance_n'][i]:15.4e}{r['InputImpedance_n'][i]:+10.4e}j")
    ...
    def validate_config(user_cfg):
        d = {
            ...
            'Numerics': {'BasisFunction': 'rooftop', 'Accuracy': {'Level': 'medium', 'AbsTol': 1e-6, 'RelTol': 1e-3}},
            'Execution': {'Frequency': 300e6, 'NumModes': 4, 'Verbose': True, 'PlotVisible': True, 'SaveOutputs': False, 'StoreZMatrix': True},
            'Convergence': {'Enabled': False, 'Metric': 'Eigenvalues', 'Tolerance': 1e-2, 'MinSegments': 21, 'MaxSegments': 81, 'Step': 10},
            'Benchmark': {'Enabled': False, 'ReferenceFile': '', 'ModeToCompare': 1}
        }
        ...
        if final_cfg['Mesh']['Segments'] % 2 == 0:
            raise ValueError("config.Mesh.Segments must be an odd integer.")
        return final_cfg
