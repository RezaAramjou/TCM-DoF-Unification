...
class CmaSolver:
    ...
    Version: 14.0 (Final, Fallback Solver)
    - Implemented the ultimate stability fix: If the robust, regularized
      Cholesky/eigh method fails, the solver now issues a warning and falls
      back to the general eigensolver `eig(X, R)`, which does not require a
      positive-definite R matrix. This prevents crashes on numerically
    ...
    def _solve_modes(self, X, R):
        ...
        np.fill_diagonal(R, np.maximum(np.diag(R), 0))
        max_r_val = np.max(np.abs(R))
        diag_loading = 1e-12 * max_r_val if max_r_val > 1e-9 else 1e-9
        ...
        try:
            ...
            warnings.warn(
                "R matrix is not positive definite. Falling back to general eigensolver `eig`. "
                "Mode orthogonality may not be guaranteed for this data point.",
                UserWarning
            )
            # Use the general, non-Hermitian eigensolver as a fallback.
            # It's slower but does not require a positive-definite R.
            vals, V = eig(X, R)

        # Eigenvalues from `eig` can be complex, take real part for sorting.
        # In theory, they should be real for this physical problem.
        if np.iscomplexobj(vals):
            warnings.warn("Eigenvalues have non-trivial imaginary parts. This is non-physical.", UserWarning)
            sort_vals = np.real(vals)
        else:
            sort_vals = vals

        idx = np.argsort(np.abs(sort_vals))
        N_modes = self.cfg['Execution']['NumModes']
        vals, V = np.real(vals[idx][:N_modes]), V[:, idx][:, :N_modes]
        
        for i in range(V.shape[1]):
            dot_product = V[:, i].conj().T @ R @ V[:, i]
            if np.real(dot_product) < 0:
                warnings.warn(f"Mode {i+1} has a non-physical negative dot product ({dot_product:.2e}) during normalization. "
                              "This indicates numerical instability. Using absolute value to proceed.", UserWarning)
                norm_factor = np.sqrt(np.abs(dot_product))
            else:
                norm_factor = np.sqrt(dot_product)
    ...
    def _decompose_Z(Z):
        ...
        resistance and reactance matrices for CMA.
        """
        Zs = (Z + Z.conj().T) / 2
        R = np.real(Zs)
        X = np.imag(Zs)
