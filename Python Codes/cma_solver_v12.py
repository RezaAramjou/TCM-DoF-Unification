...
class CmaSolver:
    ...
    Version: 12.0 (Rock-Solid, Final)
    - Corrected input impedance calculation for R-orthonormalized modes.
    ...
    def _run_single_frequency(self, f):
        ...
        Z_in = self._calculate_input_impedance(Z, J_n, zc, zn, dL, lambda_n)
    ...
    def _calculate_Z_element(self, m, n, zn, zc, dL, k, a):
        ...
        if basis == "pulse":
            ...
            if m == n:
                smooth_integrand = lambda zp: k**2 * self._green(zm, zp, k, a) - (1 / np.sqrt((zm - zp)**2 + a**2))
                ...
                term_V = (real_part + singular_analytic) + 1j * imag_part
                 ...
                 integrand = lambda zp: k**2 * self._green(zm, zp, k, a)
    ...
    def _calculate_input_impedance(self, Z, Jn, zc, zn, dL, lambda_n):
        N = Jn.shape[0]
        ...
        if basis == "pulse":
            ...
            if node_idx > 0: V[node_idx - 1] = 1.0
        
        alpha = Jn.conj().T @ V
        return np.sum(alpha**2 / (1 + 1j*lambda_n))
    ...
    def _log_summary_table(self, r):
        ...
        header = f"{'Mode':>5} {'Eigenvalue':>12} {'MS':>12} {'Q_Factor':>12} {'Directivity':>12} {'Z_in':>15}"
        print(header + "\n" + "-"*len(header))
        # Input impedance is a single value, not per mode
        Z_in_str = f"{r['InputImpedance']:>15.2f}"
        for i in range(min(10, len(r['lambda_n']))):
            print(f"{i+1:5d} {r['lambda_n'][i]:12.4g} {r['ModalSignificance'][i]:12.4g} {r['Q_n'][i]:12.4g} "
