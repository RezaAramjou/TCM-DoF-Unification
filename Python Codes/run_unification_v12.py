# run_unification_v12.py
import numpy as np
from joblib import Parallel, delayed
from tqdm import tqdm
from cma_solver_v12 import CmaSolver # Use the new solver
...
def fit_spherical_waves(theta, E_pattern, max_N, error_threshold):
    ...
    for N in range(1, max_N + 1):
        ...
        for n in range(1, N + 1):
            norm_factor = np.sqrt((2*n + 1) * np.math.factorial(n-1) / (2 * np.math.factorial(n+1)))
            A[:, n-1] = norm_factor * lpmv(1, n, costh)
        ...
        try:
            ...
            # Use a power-based error metric
            power_true = np.trapz(np.abs(E_pattern)**2 * sinth, theta)
            power_err = np.trapz(np.abs(E_pattern - E_reconstructed)**2 * sinth, theta)
            
            if (power_err / power_true) < error_threshold:
                return N
...
# --- Step 1: Define Sweep ---
print('--- Unification Analysis (v12.0 Solver) ---')
f0 = 300e6
...
for r in tqdm(results, desc="Analyzing Results"):
    ...
    E_pattern_total = AF_matrix[0,:] * np.sin(theta)
    
    if np.max(np.abs(E_pattern_total)) > 1e-9:
        N_fit = fit_spherical_waves(theta, E_pattern_total, 20, 0.05)
